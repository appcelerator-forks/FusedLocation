...
#define TAG "ca.underlabs.fusedlocation"

using namespace v8;

static Persistent<Object> bindingCache;

static void FusedlocationModule_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "FusedlocationModule.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::FusedlocationModuleBindings::lookupGeneratedInit(
...
static void FusedlocationModule_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, FusedlocationModule_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void FusedlocationModule_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::FusedlocationModuleBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry FusedlocationModuleBinding = {
	"ca.underlabs.fusedlocation",
	FusedlocationModule_init,
	FusedlocationModule_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ca_underlabs_fusedlocation_FusedlocationModuleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ca.underlabs.fusedlocation", &FusedlocationModuleBinding);
	titanium::KrollBindings::addExternalLookup(&(::FusedlocationModuleBindings::lookupGeneratedInit));
}
